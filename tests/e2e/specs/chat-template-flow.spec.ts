...
test.describe('Chat Template Flow', () => {
  ...
  test('should create a new chat from productivity template', async ({ page }) => {
    ...
    const templatePath = path.join(testVaultDir, 'Cerebro', 'Templates', 'Productivity Template.md');
    ...
    expect(updatedContent).toContain('How can I organize my tasks better for maximum productivity?');
    ...
    const newFileName = path.join(testVaultDir, 'Cerebro', 'Chats', 'Task Organization Techniques.md');
  ...
  test('should create a chat from creative writing template and handle complex content', async ({ page }) => {
    // We're simulating creating a new chat from the creative writing template
    const templatePath = path.join(testVaultDir, 'Cerebro', 'Templates', 'Creative Writing Template.md');
    ...
    // Simulate inferring a title
    const newFileName = path.join(testVaultDir, 'Cerebro', 'Chats', 'The Magic Comment - A Programmers Tale.md');
    fs.renameSync(chatFilePath, newFileName);
