...
class MockOpenAIClient {
  ...
  async inferTitle(messages: Message[], inferTitleLanguage: string): Promise<string> {
    const result = await this.client.chat.completions.create({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: `Infer a title from the messages. Write the title in ${inferTitleLanguage}.` },
        ...messages.map(m => ({ role: m.role, content: typeof m.content === 'string' ? m.content : 'Content' }))
      ],
      temperature: 0.0
    });
  ...
  appendSystemCommands(systemCommands: string[], messages: Message[]): Message[] {
    ...
    return [
      { role: 'system', content: systemCommands.join('\n') },
      ...messages
    ];
  }
...
describe('OpenAIClient', () => {
  ...
  describe('inferTitle', () => {
    ...
    it('should return the inferred title from API response', async () => {
      ...
      // Setup test data
      const messages: Message[] = [
        { role: 'user', content: '¿Cuáles son las diferencias clave entre el aprendizaje automático y el aprendizaje profundo?' },
        { role: 'assistant', content: 'El aprendizaje automático es un subconjunto de la IA...' }
      ];
  ...
  describe('chat', () => {
    it('should handle non-streaming responses correctly', async () => {
      ...
      // Call the method
      const result = await client.chat(messages, frontmatter as any, mockChatInterface as any);
      ...
      expect(mockChatInterface.appendNonStreamingMessage).toHaveBeenCalledWith('This is a test response');
    });
    ...
    it('should handle unfinished code blocks properly', async () => {
      ...
      const codeBlockResponse = {
        choices: [{
          message: { content: 'Here is an example:\n```python\ndef hello():\n    print("Hello, world!")' },
          finish_reason: 'stop'
        }]
      };
      ...
      // Setup test data
      const messages: Message[] = [
        { role: 'user', content: 'Show me a Python function.' }
      ];
      const frontmatter = {
        llm: 'OpenAI',
        model: 'gpt-4',
